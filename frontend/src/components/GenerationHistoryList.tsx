import type { ProjectGenerationListItem } from '../types/projectGeneration'

interface GenerationHistoryListProps {
  generations: ProjectGenerationListItem[]
    ...
    <div className="space-y-2">
      {generations.map((generation) => {
        ...
        return (
          ...
          >
            <div className="text-sm font-medium">{generation.project_name}</div>
            <div className="mt-1 truncate text-xs text-slate-400">{generation.idea}</div>
          </button>
        )
      })}
    </div>
